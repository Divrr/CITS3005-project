...
if has_url_property:
    # Correct way to check if property is functional in OWLReady2
    ...
    for manual in tqdm(data, desc="Processing manuals"):
        ...
        for category_name in reversed(manual["Ancestors"]):
            if not category_name:
                #print("Skipping None or empty category_name.")
        ...
        for tool_data in manual["Toolbox"]:
            ...
            if not tool_name:
                #print("Skipping None or empty tool_name.")
        ...
        for step_data in tqdm(manual["Steps"], desc="Processing steps", leave=False):
            ...
            for action_data in step_data.get("Removal_verbs", []):
                ...
                if not action_name:
                    #print("Skipping None or empty action_name.")
                    continue  # Skip if action_name is None or empty
                action_name_clean = action_name.replace(" ", "_")
            ...
            for part_name in step_data.get("Word_level_parts_clean", []):
                if not part_name:
                    #print("Skipping None or empty part_name.")
            ...
            for image_url in step_data.get("Images", []):
                if not image_url:
                    #print("Skipping None or empty image_url.")
